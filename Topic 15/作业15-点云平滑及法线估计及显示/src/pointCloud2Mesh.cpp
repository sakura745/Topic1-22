...
int main(int argc, char** argv)
{
	...
	// 法线估计
    pcl::NormalEstimation<PointT, pcl::Normal> normalEs;
    normalEs.setInputCloud(cloud_smoothed);
    pcl::search::KdTree<PointT>::Ptr treeNor (new pcl::search::KdTree<PointT> );
    normalEs.setSearchMethod(treeNor);
    normalEs.setKSearch(5);
    pcl::PointCloud<pcl::Normal>::Ptr normals (new pcl::PointCloud<pcl::Normal>);
    normalEs.compute(*normals);
	// ----------------------结束你的代码--------------------------//
	// 显示结果
