...
//    imshow("test1",im_src);
    ...
    // ----------  开始你的代码  --------------
    Mat Homo = findHomography(pts_src,data.points);//求Homographymatrix
    warpPerspective(im_src, im_temp, Homo, im_temp.size());//将src_pic转化为tmp_pic的角度
    ...
    fillConvexPoly(im_dst, pts, 4, Scalar(0,0,0));//在dst_pic上把pts位置填充
