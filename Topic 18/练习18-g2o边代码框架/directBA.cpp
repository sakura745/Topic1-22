...
public:
    ...
    virtual void computeError() override {
        ...
        // ----------------  开始你的代码 ----------------------//
		// 参考十四讲中直接法BA部分
        const VertexSBAPointXYZ* Pw = static_cast<const VertexSBAPointXYZ*> (vertex(0));
        const VertexSophus* T = static_cast<const VertexSophus*> (vertex(1));
        Vector3d P = T->estimate() * Pw->estimate();
        float u = fx * P[0] / P[2] + cx;
        float v = fy * P[0] / P[2] + cy;

        //因为bInImage是左闭右开区间，因此是 u-3 和 u+2
        if (!bInImage(u - 3, v - 3, w, h) || !bInImage(u + 2, v + 2, w, h)){
            ...
            for(int i = -2; i < 2; i++)
                for(int j = -2; j < 2; j++){
                    int num = 4 * i + j + 10;//因为i j 是从-2开始的，因此需要+10
    ...
    virtual void linearizeOplus() override
    {
        ...
        J_Puv_Pc << fx * inv_z, 0,          -fx * x * inv_z2,
...
//    optimizer.setAlgorithm ( solver );
    ...
    for(int j = 0; j < poses.size(); j++){
        VertexSophus* T = new VertexSophus();
        T->setId(j + points.size());
        T->setEstimate(poses[j]);
        optimizer.addVertex(T);
    }
    ...
    for(int i = 0; i < points.size(); i++)
        for(int j = 0; j < poses.size(); j++){
            ...
            edge->setVertex(0, dynamic_cast<VertexSBAPointXYZ*>(optimizer.vertex(i)));
