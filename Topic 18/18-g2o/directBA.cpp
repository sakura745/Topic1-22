...
public:
    ...
    virtual void computeError() override {
        ...
        // ----------------  开始你的代码 ----------------------//
		// 参考十四讲中直接法BA部分
		const VertexSophus* v1 = static_cast<const VertexSophus*>(_vertices[1]);
		const VertexSBAPointXYZ* v2 = static_cast<const VertexSBAPointXYZ*>(_vertices[0]);
        Eigen::Vector3d x_local = v1->estimate()*v2->estimate();
        ...
        for(int i = -2; i < 2; i++)
            for(int j = -2; j < 2; j++)
            {
                if((x+i<0) || (x+i) > w || (y+i<0) || (y+i) > h)
                {
                    ...
                    error(cnt, 0) = GetPixelValue(targetImg, x+i, y+j) - origColor[cnt];
                cnt++;
            }
        _error = error;
...
//    optimizer.setAlgorithm ( solver );
	...
	for(int i = 0; i < images.size(); i++)
	    for(int j = 0; j < color.size(); j++)
        {
            ...
            edge->setId(index++);
            edge->setVertex(0, dynamic_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(poses.size()+j)));
            edge->setVertex(1, dynamic_cast<VertexSophus*>(optimizer.vertex(i)));
            edge->setMeasurement ( Vector16d(0) );
