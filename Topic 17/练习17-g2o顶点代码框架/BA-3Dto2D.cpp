        ...
        Mat& K   )
{
    ...
    std::unique_ptr<Block::LinearSolverType> linear_slover (new g2o::LinearSolverCSparse<Block::PoseMatrixType>());

    // 第2步：创建BlockSolver。并用上面定义的线性求解器初始化
    std::unique_ptr<Block> solver_ptr (new Block(std::move(linear_slover)));
